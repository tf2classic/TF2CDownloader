...
def tf2c_download():
    ...
    run([vars.ARIA2C_BINARY, '--max-connection-per-server=16', '-UTF2CDownloader2022-07-31', '--max-concurrent-downloads=16', '--optimize-concurrent-downloads=true', '--check-certificate=false', '--check-integrity=true', '--auto-file-renaming=false', '--continue=true', '--console-log-level=error', '--summary-interval=0', '--bt-hash-check-seed=false', '--seed-time=0',
    '-d' + vars.TEMP_PATH,
